...
def preprocessing(data_dataframe):
    ...
    # preprocessing 2
    data_dataframe = data_dataframe.drop(["TWILIO_Ligações Inbound", "TWILIO_Data Última Ligações Inbound","COBRANÇA_VINDI_Qde Total de Faturas","COBRANÇA_VINDI_Qde Total de Tentativas de Cobrança","COBRANÇA_VINDI_Método de Pagamento","COBRANÇA_VINDI_Valor Médio da Mensalidade","COBRANÇA_VINDI_Qde Total de Faturas Pagas após Vencimento","COBRANÇA_VINDI_Qde Total de Faturas Inadimpletes","COBRANÇA_VINDI_Valor Total Inadimplência"], axis=1)
    ...
    for indice, valor in data_dataframe["FUNIL_ASSINATURA_PIPEDRIVE_lost_time"].items():
        ...
        data_dataframe['ATENDIMENTOS_AGENDA_Faltas Psicoterapia'] = data_dataframe['ATENDIMENTOS_AGENDA_Faltas Psicoterapia'].fillna(0)
        data_dataframe['ATENDIMENTOS_AGENDA_Qde Prescrições'] = data_dataframe['ATENDIMENTOS_AGENDA_Qde Prescrições'].fillna(0)
